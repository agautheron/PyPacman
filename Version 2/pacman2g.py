...
import sys, os
from pacman2c import *
import pacman2 as pacman
...
class CellWidget(QLabel):
    ...
    def refresh(self,value):
        if type(value):
            self.value=value
            if value=='xxx ':
                self.path='Data\\'+self.value[:-1].upper()+'.png'
            if value[:2]=='G.':
            self.path='Data\\'+self.value[:-1]+'.png'
...
class HomeWindow(QMainWindow):
    ...
    def game_menu(self):
        ...
        gameMenuStartAction.setShortcut("Ctrl+Enter")
        gameMenuStartAction.setStatusTip('Demarrer la partie')
        gameMenuStartAction.triggered.connect(self.startGame)
