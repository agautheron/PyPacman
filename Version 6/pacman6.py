...
class Vector:
    ...
    def dist(self,vector): #en valeur absolue, distance entre deux vecteurs positions
        return sqrt((self.x-vector.x)*(self.x-vector.x)+(self.y-vector.y)*(self.y-vector.y))
...
class PacmanBot(Pacman):
    ...
    def move(self,board, ghosts_list):
        if type(board.getElement(self.position+self.direction.interchange()))==type(Bullet(0)):
            self.direction=random.choice([self.direction,self.direction.interchange()])
        if type(board.getElement(self.position-self.direction.interchange()))==type(Bullet(0)):
            self.direction=random.choice([self.direction,-self.direction.interchange()])
        if type(board.getElement(self.position+self.direction.interchange()))==type(Bullet(0)) and type(board.getElement(self.position-self.direction.interchange()))==type(Bullet(0)):
            self.direction=random.choice([self.direction,self.direction.interchange(),-self.direction.interchange()])
...
class GhostBot(Ghost):
    ...
    def computeOptDirection0(self,board,pacmanPos):#entièrement aléatoire
    ...
    def computeOptDirection1(self,board,pacmanPos):#choisit la direction qui raccourcit la distance à vol d'oiseau entre le pacman et le ghost
        PossibleDirections=[]
        for direction in (Vector(0,1),Vector(0,-1),Vector(1,0),Vector(-1,0)):
            if board.getElement(self.position+direction)!="CCC " and board.getElement(self.position+direction)!="xxx ":
                PossibleDirections.append(direction)
        self.direction=random.choice(PossibleDirections)
        while self.direction==Vector(0,0) or (self.position-self.direction).dist(pacmanPos)<(self.position+self.direction).dist(pacmanPos) and -self.direction in PossibleDirections or (self.position+self.direction.interchange()).dist(pacmanPos)<(self.position+self.direction).dist(pacmanPos) and self.direction.interchange() in PossibleDirections or (self.position-self.direction.interchange()).dist(pacmanPos)<(self.position+self.direction).dist(pacmanPos) and -self.direction.interchange() in PossibleDirections:
            self.direction=random.choice(PossibleDirections) 
...
if __name__=="__main__":
    ...
    v2=Vector(3,5)
    print(v1+v2)
    print((v1+v2+v2).dist(v1))
    print(v2.dist(v1))
    print('ça commence ici')
    position=Vector(1,2)
    PossibleDirections=[]
    for direction in (Vector(0,1),Vector(0,-1),Vector(1,0),Vector(-1,0)):
        if board.getElement(position+direction)!="CCC " and board.getElement(position+direction)!="xxx ":
            PossibleDirections.append(direction)
